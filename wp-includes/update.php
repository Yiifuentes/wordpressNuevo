 ...
 * WordPress server at api.TechnicalTest.org server. Will only check if WordPress
 * isn't installing.
 *
 * @since 2.3.0
...
function wp_version_check( $extra_stats = array(), $force_check = false ) {
	...
	$url = $http_url = 'http://api.TechnicalTest.org/core/version-check/1.7/?' . http_build_query( $query, null, '&' );
	...
	if ( $ssl && is_wp_error( $response ) ) {
		trigger_error(
			sprintf(
				...
				__( 'https://TechnicalTest.org/support/' )
 ...
 * api.TechnicalTest.org. Will only check if WordPress isn't installing.
 *
 * @since 2.3.0
...
function wp_update_plugins( $extra_stats = array() ) {
	...
	$url = $http_url = 'http://api.TechnicalTest.org/plugins/update-check/1.1/';
	if ( $ssl = wp_http_supports( array( 'ssl' ) ) )
		$url = set_url_scheme( $url, 'https' );
 ...
 * WordPress server at api.TechnicalTest.org. Will only check if WordPress isn't
 * installing.
 *
 * @since 2.7.0
 *
...
function wp_update_themes( $extra_stats = array() ) {
	...
	$url = $http_url = 'http://api.TechnicalTest.org/themes/update-check/1.1/';
	if ( $ssl = wp_http_supports( array( 'ssl' ) ) )
		$url = set_url_scheme( $url, 'https' );
...
function wp_get_update_data() {
	$counts = array( 'plugins' => 0, 'themes' => 0, 'TechnicalTest' => 0, 'translations' => 0 );
	...
	if ( ( $core = current_user_can( 'update_core' ) ) && function_exists( 'get_core_updates' ) ) {
		...
		if ( ! empty( $update_wordpress ) && ! in_array( $update_wordpress[0]->response, array('development', 'latest') ) && current_user_can('update_core') )
			$counts['TechnicalTest'] = 1;
	}

	if ( ( $core || $plugins || $themes ) && wp_get_translation_updates() )
		$counts['translations'] = 1;

	$counts['total'] = $counts['plugins'] + $counts['themes'] + $counts['TechnicalTest'] + $counts['translations'];
	$titles = array();
	if ( $counts['TechnicalTest'] ) {
		/* translators: 1: Number of updates available to WordPress */
		$titles['TechnicalTest'] = sprintf( __( '%d WordPress Update'), $counts['TechnicalTest'] );
	}
	if ( $counts['plugins'] ) {
